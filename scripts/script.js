...
onAuthStateChanged(auth, async (user) => {
  ...
  const userGameHistory = await getData(
    `${API_URL_USERS}/${currentUser.id}/user-game-data`
  );
...
async function timer() {
  ...
  intervalId = setInterval(() => {
    if (!+time.toFixed(2)) {
      selectedTime.value = (0.0).toFixed(2);
      ...
      (async () => {
        ...
        updateData(
          `${API_URL_USERS}/${currentUser.id}/user-game-data/${currentUser.id}`,
          {
...
function hanlder(event) {
  game_container.addEventListener("dragstart", (event) =>
    event.preventDefault()
  );

  if (!event.target.closest(".game-cards__card")) return;
  if (!game_timer.hasAttribute("disabled"))
    game_timer.setAttribute("disabled", "disabled");
