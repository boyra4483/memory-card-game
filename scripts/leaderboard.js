    ...
    for (let i = 0; i < users_data.length; i++) {
      ...
      if (!i) {
        if (document.querySelectorAll(".user-data").length < user_pts.length) {
          for (
            let i = document.querySelectorAll(".user-data").length;
            i < user_pts.length;
            i++
          ) {
            document
              .querySelector(".users-list")
              .append(list_items[1].cloneNode(true));
          }
        } else {
          while (true) {
            if (
              document.querySelectorAll(".user-data").length == user_pts.length
            )
              break;
            document
              .querySelectorAll(".user-data")
              [document.querySelectorAll(".user-data").length - 1].remove();
          }
        }
      } else {
        const list_items = document.querySelectorAll(".user-data");
        while (true) {
          if (
            list_items.length + user_pts.length ==
            document.querySelectorAll(".user-data").length
          )
            break;
          document
            .querySelector(".users-list")
      ...
      for (let j = 0; j < user_pts.length; j++) {
        ...
        list_item.querySelector(".user-data__name").textContent =
          user_data[0].name;

        list_item.querySelector(".user-data__speed").textContent =
          user_data[1].pts[j];
        list_item.querySelector(".user-data__guessed-cards").textContent =
          user_data[1].guessedCard[j];

        if (!j) continue;
        list_item.querySelector(".user-data__position").textContent = `${
          j + 1
        }`;
